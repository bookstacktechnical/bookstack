...
import { GetUserById } from "../../../backendapicalls/users";
import { HideLoading, ShowLoading } from "../../../redux/loadersSlice";
import { EditIssue, IssueBook } from "../../../backendapicalls/issues";
    ...
    try {
      ...
      if (response.success) {
        if (response.data.role !== "customer") {
          setValidated(false);
          setErrorMessage("This user is not a customer");
      ...
      <div className="flex flex-col gap-2">
        ...
        {validated && (
          <div className="bg-secondary p-1 text-white">
            <h1 className="text-sm">Customer : {customerData.name}</h1>
            <h1>
        ...
        <div className="flex justify-end gap-2 w-100">
          ...
          {type === "add" && (
            <Button
              title="Validate"
              disabled={customerId === "" || returnDate === ""}
              onClick={validate}
            />
          )}
          {validated && (
